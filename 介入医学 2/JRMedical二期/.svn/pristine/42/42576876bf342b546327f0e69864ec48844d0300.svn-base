...
- (void)awakeFromNib {
    ...
    // add
//    _countLabel= [[UILabel alloc] initWithFrame:(CGRectMake(Width_Screen-85, 0, 80, 10))];
    ...
    if ([lang isEqualToString:@"zh-Hans"]) { // 简体中文输入，包括简体拼音，健体五笔，简体手写
        ...
        if (!position) {
            if (self.text.length > self.maxTextLength) {
                [SVProgressHUD showErrorWithStatus:@"字数限制为300字"];
...
//    self.countLabel.text = [NSString stringWithFormat:@"%lu / %d", (unsigned long)self.text.length, 300];
