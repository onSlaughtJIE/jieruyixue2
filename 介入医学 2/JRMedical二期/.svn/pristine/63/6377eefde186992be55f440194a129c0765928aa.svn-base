...
//    }
    [self getUserIsDoctor];
    
//    NSMutableAttributedString *attStr = [[NSMutableAttributedString alloc]initWithString:@"官方提醒：我是收费用户"];
//    NSRange range = [[attStr string]rangeOfString:@"官方提醒：我是收费用户"];
//    [attStr addAttribute:NSForegroundColorAttributeName value:[UIColor redColor] range:range];
    
//    [self sendTextMessage:_moneyStr];
...
- (void)getUserIsDoctor {
    NSLog(@"^^^^^^^^^%@", _userid);
    NSString *urlS = [NSString stringWithFormat:@"%@/api/IM/IsDoctorByID", Server_Int_Url];
    ...
    [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
        NSLog(@"IsDoctorByID - %@", responseObjeck);
...
//        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:clearButton];
        UIButton *setupButton = [[UIButton alloc] initWithFrame:CGRectMake(0, 0, 44, 44)];
        [setupButton setImage:[UIImage imageNamed:@"shezhi"] forState:UIControlStateNormal];
        [setupButton addTarget:self action:@selector(enterSetup:) forControlEvents:UIControlEventTouchUpInside];
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithCustomView:setupButton];
    }
    else{//群聊
        self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc]initWithImage:[UIImage imageNamed:@"group_detail"] style:(UIBarButtonItemStylePlain) target:self action:@selector(showGroupDetailAction)];
    }
}

- (void)enterSetup:(UIButton *)sender {
    
    ChaterSetViewController *setVC = [[ChaterSetViewController alloc] init];
    setVC.userid = self.userid;
   ...
   didSelectAvatarMessageModel:(id<IMessageModel>)messageModel
{
    ...
    if ([AFManegerHelp isYourFriendsWith:messageModel.message.from]) {
        
        // 是好友
        YDetailController *yVC = [[YDetailController alloc] init];
        yVC.userid = messageModel.message.from;
        ...
        mdVC.userid = messageModel.message.from;
...
//    }
    model.avatarImage = [UIImage imageNamed:@"chatListCellHead@2x"];
...
//        }
        ...
        NSLog(@"target.userId - %@", target.userId);
        
        NSString *urlS = [NSString stringWithFormat:@"%@/api/IM/GetCustomerInfoByID", Server_Int_Url];
        NSString *dataS = [NSString stringWithFormat:@"%@DevIdentity=%@ZICBDYCDevSysInfo=%@ZICBDYCDevTypeInfo=%@ZICBDYCIMEI=%@ZICBDYCID=%@", kPrefixPara, kDevIdentity, kDevSysInfo, kDevTypeInfo, kIMEI, target.userId, nil];
        NSString *dataEncrpyt = [TWDes encryptWithContent:dataS type:kDesType key:kDesKey];
        NSString *token = [UserInfo getAccessToken];
        NSDictionary *paraDic = @{kToken:token, kDatas:dataEncrpyt};
        ...
        [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
            ...
            if ([responseObjeck[@"Success"] integerValue] == 1) {
                ...
                NSString *name = dic[@"CustomerName"];
