...
//    picker.modalPresentationStyle = UIModalPresentationCurrentContext;
    ...
    //设置拍摄的时间
    picker.videoMaximumDuration = 30; // 最大录制时长
...
//            self.assetsFilter = [ALAssetsFilter allVideos];
            ...
            self.labelSelectedMedia.hidden = NO;
            if(_maximumNumberOfSelection > 1)
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"请选择视频", @"UzysAssetsPickerController", nil);
            else
                self.labelSelectedMedia.text = NSLocalizedStringFromTable(@"请选择视频", @"UzysAssetsPickerController", nil);
        }
        else if(_maximumNumberOfSelectionVideo == 0)
...
- (void)setupGroup:(voidBlock)endblock withSetupAsset:(BOOL)doSetupAsset
{
    ...
    ALAssetsLibraryAccessFailureBlock failureBlock = ^(NSError *error) {
        ...
        [strongSelf setTitle:NSLocalizedStringFromTable(@"不允许", @"UzysAssetsPickerController",nil)];
...
- (void)initNoAssetView
{
    ...
    title.text              = NSLocalizedStringFromTable(@"没有图片或视频", @"UzysAssetsPickerController", nil);
    ...
    message.text            = NSLocalizedStringFromTable(@"可以通过iTunes同步视频或图片", @"UzysAssetsPickerController",nil);
...
- (void)showNotAllowed
{
    ...
    title.text              = NSLocalizedStringFromTable(@"没有权限访问相册", @"UzysAssetsPickerController",nil);
    title.font              = [UIFont boldSystemFontOfSize:17.0];
    title.textColor         = [UIColor colorWithRed:129.0/255.0 green:136.0/255.0 blue:148.0/255.0 alpha:1];
    title.textAlignment     = NSTextAlignmentCenter;
    title.numberOfLines     = 5;
    
    message.text            = NSLocalizedStringFromTable(@"进入设置授权进入相册", @"UzysAssetsPickerController",nil);
...
- (void)showNoAssetsIfNeeded
{
    ...
    voidBlock setNoImage = ^{
        ...
        msg.text = NSLocalizedStringFromTable(@"可以通过iTunes同步图片.",@"UzysAssetsPickerController", nil);
    };
    ...
    if(self.assets.count ==0)
    {
        ...
        else
        {
            if(self.maximumNumberOfSelectionMedia >0)
            {
                ...
                title.text = NSLocalizedStringFromTable(@"没有文件", @"UzysAssetsPickerController",nil);
                UILabel *msg = (UILabel *)[self.noAssetView viewWithTag:kTagNoAssetViewMsgLabel];
                msg.text = NSLocalizedStringFromTable(@"你可以通过iTunes同步文件.",@"UzysAssetsPickerController", nil);
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //限制选择视频的长度
    NSLog(@"视频长度-----%f",[[selectedAsset valueForProperty:ALAssetPropertyDuration] doubleValue]);
    if ([[selectedAsset valueForProperty:ALAssetPropertyDuration] intValue] > 30) {
        MBProgressHUD *hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
        
        hud.mode = MBProgressHUDModeText;
        
        hud.labelText = @"不能超过30秒录制时间";
...
- (IBAction)btnAction:(id)sender {
    ...
    switch (btn.tag) {
        ...
        {
            if (![UIImagePickerController isSourceTypeAvailable:UIImagePickerControllerSourceTypeCamera]) {
                NSString *title = NSLocalizedStringFromTable(@"提示", @"UzysAssetsPickerController", nil);
                NSString *message = NSLocalizedStringFromTable(@"设备没有相机", @"UzysAssetsPickerController", nil);
