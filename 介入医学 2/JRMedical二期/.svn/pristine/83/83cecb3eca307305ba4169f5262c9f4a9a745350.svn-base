...
@interface EaseMessageViewController ()<EaseMessageCellDelegate, WechatShortVideoDelegate>
{
 ...
 *
 */
- (void)moreViewListVideoAction:(EaseChatBarMoreView *)moreView
{
    // 隐藏键盘
    [self.chatToolbar endEditing:YES];
    ...
    wechatShortVideoController.isLong = NO;
...
- (void)finishWechatShortVideoCapture:(NSURL *)filePath {
    ...
    NSLog(@"小视频代理");
    [self sendVideoMessageWithURL:filePath];
...
- (NSArray *)formatMessages:(NSArray *)messages
{
    ...
    for (EMMessage *message in messages) {
        ...
        if (_dataSource && [_dataSource respondsToSelector:@selector(messageViewController:modelForMessage:)]) {
            ...
            NSString *dataS = [NSString stringWithFormat:@"%@DevIdentity=%@ZICBDYCDevSysInfo=%@ZICBDYCDevTypeInfo=%@ZICBDYCIMEI=%@ZICBDYCID=%@", kPrefixPara, kDevIdentity, kDevSysInfo, kDevTypeInfo, kIMEI,message.conversationId, nil];
            ...
            [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
                ...
                if ([responseObjeck[@"Success"] integerValue] == 1) {
                    ...
                    for (NSDictionary *d in array) {
                        model.nickname = d[@"CustomerName"];
                        NSLog(@"EaseMessageViewController - model%@", model.nickname);
                    }
                }
