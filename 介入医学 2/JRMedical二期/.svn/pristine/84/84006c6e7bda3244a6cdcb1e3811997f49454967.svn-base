...
@implementation MyBalanceDetailedListCell
...
- (void)setModel:(MyBalanceDetailedListModel *)model {
    _model = model;

    NSString *yearStr = [model.CreateTime substringWithRange:NSMakeRange(0,4)];
    NSString *monthStr = [model.CreateTime substringWithRange:NSMakeRange(5,5)];
    ...
    self.titleLab.text = model.RecordTypeName;
    
    self.numLab.text = [NSString stringWithFormat:@"%@ : ¥ %@",model.PayTypeName,model.RecordAmount];
