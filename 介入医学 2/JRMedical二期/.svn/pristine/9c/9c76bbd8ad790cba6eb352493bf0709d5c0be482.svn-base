...
@interface YDetailController ()<UIAlertViewDelegate, LGAlertViewDelegate, UITextFieldDelegate>
...
@implementation YDetailController
...
- (void)getUSerMessage {
    ...
    [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
        ...
        NSLog(@"YdetailController - GetCustomerInfoByID - %@", responseObjeck);
...
- (IBAction)chatwithmember:(UIButton *)sender {
    if ([[EMClient sharedClient].currentUsername isEqualToString:_userid]) {
        NSLog(@"自己");
        
        UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"你不能和自己聊天" message:nil delegate:self cancelButtonTitle:@"好的" otherButtonTitles:nil, nil];
        [alert show];
        
    }
    
    else if (![AFManegerHelp isYourFriendsWith:_userid]) {
        NSLog(@"是好友");
        ...
        ChatViewController *chatController = [[ChatViewController alloc]
                                              initWithConversationChatter:_userid conversationType:EMConversationTypeChat];
        
        chatController.title = _nickname;
        chatController.userid = _userid;
...
// 加入黑名单
- (IBAction)pullBlackListAction:(UIButton *)sender {
