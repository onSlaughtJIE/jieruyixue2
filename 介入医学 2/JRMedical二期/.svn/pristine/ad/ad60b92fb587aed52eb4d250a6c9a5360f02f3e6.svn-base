...
// add
//@property (nonatomic, strong) NSString *currentNickName; // 当前环信用户的昵称
                    ...
                    isPublish:(BOOL)isPublish
{
    ...
    if (self) {
        ...
        self.printScreenButton.hidden = YES;
...
//        [self getUserNickNameWithEMID:from];
    }
    return self;
}
                            ...
                            username:(NSString *)aUsername
{
    ...
    [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
        ...
        if ([responseObjeck[@"Success"] integerValue] == 1) {
            ...
            NSString *from = dic[@"CustomerName"];
            ...
            EMMessage *message = [[EMMessage alloc] initWithConversationID:aChatroom.chatroomId from:from to:aChatroom.chatroomId body:body ext:@{@"em_join":aUsername}];
            message.chatType = EMChatTypeChatRoom;
            [self.datasource addObject:message];
            [self.tableView reloadData];
            [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:[self.datasource count] - 1 inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:YES];
                            ...
                            username:(NSString *)aUsername
{
    ...
    [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
        ...
        if ([responseObjeck[@"Success"] integerValue] == 1) {
            ...
            EMMessage *message = [[EMMessage alloc] initWithConversationID:aChatroom.chatroomId from:from to:aChatroom.chatroomId body:body ext:@{@"em_leave":aUsername}];
            message.chatType = EMChatTypeChatRoom;
            [self.datasource addObject:message];
            [self.tableView reloadData];
            [self.tableView scrollToRowAtIndexPath:[NSIndexPath indexPathForRow:[self.datasource count] - 1 inSection:0] atScrollPosition:UITableViewScrollPositionBottom animated:YES];
...

{
    ...
    NSString *from = @"";
    if (self.currentNickName.length > 0) {
        from = self.currentNickName;
    } else {
...
- (void)getUserNickNameWithEMID:(NSString *)EMID {
    ...
    [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
        ...
        if ([responseObjeck[@"Success"] integerValue] == 1) {
            ...
            NSArray *array = responseObjeck[@"JsonData"];
            NSDictionary *dic = array.firstObject;
            self.currentNickName = dic[@"CustomerName"];
        }
...
- (BOOL)leaveChatroom
{
    ...
    if (!error) {
        ...
        [[EMClient sharedClient].chatManager deleteConversation:_chatroomId isDeleteMessages:YES completion:nil];
        return YES;
    }
    return NO;
}
