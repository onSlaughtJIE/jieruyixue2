...
@interface MyPostVC ()<UITableViewDelegate,UITableViewDataSource,YHWorkGroupPhotoContainerDelegate,NewesPostFooterViewDelegate>
{
    NSString *_postID;
    NSIndexPath *_index;
}
...
- (void)requestListDataArrray {
    ...
    NSString *url = @"api/Customer/MyPostLst";
    NSString *params = @"";
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    MyPostTieZiCell *cell = [tableView dequeueReusableCellWithIdentifier:NSStringFromClass([MyPostTieZiCell class]) forIndexPath:indexPath];
    ...
    [cell.deleteBtn addTarget:self action:@selector(deleteTiezi:) forControlEvents:(UIControlEventTouchUpInside)];
...
//
- (void)deleteTiezi:(UIButton *)sender {
    
    MyPostTieZiCell *cell = (MyPostTieZiCell *)sender.superview.superview;
    _index = [self.tableView indexPathForCell:cell];
    NewestPostModel *model = self.dataSource[_index.row];
    _postID = model.PostID;
    
    UIAlertView *alert = [[UIAlertView alloc] initWithTitle:nil message:@"确定删除这条帖子吗" delegate:self cancelButtonTitle:@"否" otherButtonTitles:@"是", nil];
 ...
 *  删除帖子
 */
- (void)deleteMyPostTiezi {
    
    NSString *urlS = [NSString stringWithFormat:@"%@/api/Post/DeletePost", Server_Int_Url];
    NSString *dataS = [NSString stringWithFormat:@"%@DevIdentity=%@ZICBDYCDevSysInfo=%@ZICBDYCDevTypeInfo=%@ZICBDYCIMEI=%@ZICBDYCPostID=%@", kPrefixPara, kDevIdentity, kDevSysInfo, kDevTypeInfo, kIMEI, _postID, nil];
...
- (UITableView *)tableView {
    if (!_tableView) {
        ...
        [_tableView registerClass:[MyPostTieZiCell class] forCellReuseIdentifier:NSStringFromClass([MyPostTieZiCell class])];
    }
    return _tableView;
}
