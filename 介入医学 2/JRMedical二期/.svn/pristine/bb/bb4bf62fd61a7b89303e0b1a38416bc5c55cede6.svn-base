...
//    _nameLabel.text = model.nickname;
    ...
    [self getUSerMessageWith:model.nickname]; // motify jia
    ...
    [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
        ...
        if ([responseObjeck[@"Success"] integerValue] == 1) {
            ...
            for (NSDictionary *d in array) {
                _nameLabel.text = d[@"CustomerName"];
                [self.avatarView sd_setImageWithURL:[NSURL URLWithString:d[@"UserPic"]] placeholderImage:[UIImage imageNamed:@"chatListCellHead"]];
            }
        }
