...
#import "ForecastSubViewController.h"
...
@interface LiveForecastViewController ()
{
...
@implementation LiveForecastViewController
...
- (void)viewDidLoad {
    ...
    WeakSelf;
    self.passArr = ^(NSMutableArray *titleArray, NSMutableArray *groupCodeArray, NSMutableArray *valueArray, NSMutableArray *lableValueLst){
        for (int i = 0; i < titleArray.count; i++) {
            ForecastSubViewController *tabVC = [[ForecastSubViewController alloc]init];
...
- (void)requestCategoryData {
    ...
    [BaseNetwork postLoadDataApi:url withParams:params block:^(int code, BOOL isSuccess, NSDictionary *modelData) {
        ...
        if (isSuccess) {
            ...
            self.passArr(self.nameArray, self.groupCodeArray, self.valueArray, self.lableValueLst);
        }
        else {
            if (code == 999) {
