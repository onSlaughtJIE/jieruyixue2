...
@interface PatientDetailVC ()<UITableViewDelegate,UITableViewDataSource,UIWebViewDelegate,YHWorkGroupPhotoContainerDelegate>
...
- (void)viewDidLoad {
    ...
    NSLog(@"//缩略图URL - %@", self.picUrlArray);
    NSLog(@"//原图url - %@", self.picOriArray);
    NSLog(@"//图片类型 - %@", self.rTypeArray);
}
...
- (void)getReplyList {
    ...
    NSString *params = [NSString stringWithFormat:@"ZICBDYCPostID=%@",self.model.PostID];
...
- (void)shareSuccess {
    ...
    NSString *params = [NSString stringWithFormat:@"ZICBDYCResourcesType=%@ZICBDYCResourcesID=%@ZICBDYCShareGoal=%@",@"2",self.model.PostID,_shareType];
