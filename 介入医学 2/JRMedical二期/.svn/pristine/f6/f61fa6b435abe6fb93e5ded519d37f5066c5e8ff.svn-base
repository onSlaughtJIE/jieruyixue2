...
@property (nonatomic, strong) UIButton *listvideoCallButton;
...
- (void)setupSubviewsForType:(EMChatToolbarType)type
{
    ...
    [_photoButton setImage:[UIImage imageNamed:@"tupiannn"] forState:UIControlStateNormal];
    [_photoButton setImage:[UIImage imageNamed:@"tupiannn"] forState:UIControlStateHighlighted];
    [_photoButton addTarget:self action:@selector(photoAction) forControlEvents:UIControlEventTouchUpInside];
    _photoButton.tag = MOREVIEW_BUTTON_TAG;
    [_scrollview addSubview:_photoButton];
    
    _one = [[UILabel alloc]initWithFrame:CGRectMake(insets, 15 + CHAT_BUTTON_SIZE, CHAT_BUTTON_SIZE , 15)];
    _one.text = @"照片";
    _one.font = [UIFont systemFontOfSize:13];
    _one.textColor = [UIColor grayColor];
    ...
    [_locationButton setImage:[UIImage imageNamed:@"weizhi"] forState:UIControlStateNormal];
    [_locationButton setImage:[UIImage imageNamed:@"weizhi"] forState:UIControlStateHighlighted];
    [_locationButton addTarget:self action:@selector(locationAction) forControlEvents:UIControlEventTouchUpInside];
    _locationButton.tag = MOREVIEW_BUTTON_TAG + 1;
    [_scrollview addSubview:_locationButton];
    
    _two = [[UILabel alloc]initWithFrame:CGRectMake(insets * 2 + CHAT_BUTTON_SIZE, 15 + CHAT_BUTTON_SIZE, CHAT_BUTTON_SIZE , 15)];
    _two.text = @"位置";
    _two.font = [UIFont systemFontOfSize:13];
    _two.textColor = [UIColor grayColor];
    _two.textAlignment = NSTextAlignmentCenter;
    [_scrollview addSubview:_two];
    
    // 相机
    _takePicButton =[UIButton buttonWithType:UIButtonTypeCustom];
    [_takePicButton setFrame:CGRectMake(insets * 3 + CHAT_BUTTON_SIZE * 2, 10, CHAT_BUTTON_SIZE , CHAT_BUTTON_SIZE)];
    [_takePicButton setImage:[UIImage imageNamed:@"xiangjinn"] forState:UIControlStateNormal];
    [_takePicButton setImage:[UIImage imageNamed:@"xiangjinn"] forState:UIControlStateHighlighted];
    [_takePicButton addTarget:self action:@selector(takePicAction) forControlEvents:UIControlEventTouchUpInside];
    _takePicButton.tag = MOREVIEW_BUTTON_TAG + 2;
    _maxIndex = 2;
    [_scrollview addSubview:_takePicButton];
    
    _three = [[UILabel alloc]initWithFrame:CGRectMake(insets * 3 + CHAT_BUTTON_SIZE * 2, 15 + CHAT_BUTTON_SIZE, CHAT_BUTTON_SIZE , 15)];
    _three.text = @"拍照";
    _three.font = [UIFont systemFontOfSize:13];
    _three.textColor = [UIColor grayColor];
    _three.textAlignment = NSTextAlignmentCenter;
    [_scrollview addSubview:_three];
    
    // 小视频
    _listvideoCallButton =[UIButton buttonWithType:UIButtonTypeCustom];
    [_listvideoCallButton setFrame:CGRectMake(insets * 4 + CHAT_BUTTON_SIZE * 3, 10, CHAT_BUTTON_SIZE , CHAT_BUTTON_SIZE)];
    [_listvideoCallButton setImage:[UIImage imageNamed:@"xiaoshipin"] forState:UIControlStateNormal];
    [_listvideoCallButton setImage:[UIImage imageNamed:@"xiaoshipin"] forState:UIControlStateHighlighted];
    [_listvideoCallButton addTarget:self action:@selector(listbtAction) forControlEvents:UIControlEventTouchUpInside];
    _listvideoCallButton.tag = MOREVIEW_BUTTON_TAG + 6;
    _maxIndex = 4;
    [_scrollview addSubview:_listvideoCallButton];
    
    _four = [[UILabel alloc]initWithFrame:CGRectMake(insets * 4 + CHAT_BUTTON_SIZE * 3, 15 + CHAT_BUTTON_SIZE, CHAT_BUTTON_SIZE , 15)];
    _four.text = @"小视频";
    _four.font = [UIFont systemFontOfSize:13];
    _four.textColor = [UIColor grayColor];
...
- (void)listbtAction {
    NSLog(@"小视频");
    if (_delegate && [_delegate respondsToSelector:@selector(moreViewListVideoAction:)]) {
        [_delegate moreViewListVideoAction:self];
    }
}
