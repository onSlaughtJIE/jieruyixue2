...
@property (nonatomic, strong) NSMutableArray *hxinArr;
...
- (NSMutableArray *)hxinArr {
    if (_hxinArr == nil) {
        self.hxinArr = [NSMutableArray arrayWithCapacity:0];
    }
    return _hxinArr;
}
...
- (NSMutableArray *)addArray {
    if (_addArray == nil) {
        self.addArray = [NSMutableArray arrayWithCapacity:0];
    }
    return _addArray;
}
...
{
    NSLog(@"%@", blockUsernames);
    self = [self initWithNibName:nil bundle:nil];
    if (self) {
        _blockSelectedUsernames = [NSMutableArray array];
        NSString *options = [blockUsernames componentsJoinedByString:@","];
        NSLog(@"======%@", options);
        [self getUSerMessageWithOptions:options];
    }
...
//        [_contactsSource addObjectsFromArray:contacts];
        _blockSelectedUsernames = [NSMutableArray array];
        NSString *options = [contacts componentsJoinedByString:@","];
        NSLog(@"======%@", options);
        
        
        [self getUSerMessageWith:options];
...
- (void)getUSerMessageWithOptions:(NSString *)str {
    ...
    [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
        NSLog(@"ContactSelectionViewController - GetCustomerInfoByID - %@", responseObjeck);
        if ([responseObjeck[@"Success"] integerValue] == 1) {
            NSArray *array = responseObjeck[@"JsonData"];
            for (NSDictionary *d in array) {

                [self.blockSelectedUsernames addObject:d[@"CustomerName"]];
...
{
    if (_searchBar == nil) {
        _searchBar = [[EMSearchBar alloc] initWithFrame: CGRectMake(0, 0, Width_Screen, 0)];
...
{
    if (_footerView == nil) {
        _footerView = [[UIView alloc] initWithFrame:CGRectMake(0, self.view.frame.size.height - 50, Width_Screen, 50)];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSLog(@"//////0%@",_dataSource);
    NSString *imageurl = _imageArr[indexPath.row];
    NSLog(@"============*********%@", string);
...
//     NSArray *buddyList = [[EMClient sharedClient].contactManager getContactsFromDB];
    NSString *string = _hxinArr[indexPath.row];
    NSLog(@"%@", string);
    if (![self.selectedContacts containsObject:object])
    {
        ...
        [self.addArray addObject:string];
...
{   NSLog(@"取消了");
    NSString *username = [_dataSource objectAtIndex:indexPath.row];
    NSString *string = _hxinArr[indexPath.row];
    NSLog(@"%@", string);
    ...
    if ([self.selectedContacts containsObject:username]) {
        ...
        [self.addArray removeObject:string];
...
- (void)loadDataSource
{
    if (!_fromAt) {
        ...
        NSString *nameStr = [buddyList componentsJoinedByString:@","];
        
        [self getUSerMessageWith:nameStr];
    }
...
- (void)getUSerMessageWith:(NSString *)str {
    ...
    [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
        ...
        NSLog(@"ContactSelectionViewController - IM/GetCustomerInfoByID - %@", responseObjeck);
        if ([responseObjeck[@"Success"] integerValue] == 1) {
            NSArray *array = responseObjeck[@"JsonData"];
            for (NSDictionary *d in array) {

                [self.contactsSource addObject:d[@"CustomerName"]];
                [self.imageArr addObject:d[@"UserPic"]];
                [self.hxinArr addObject:d[@"ID"]];
                NSLog(@"%@", _contactsSource);
            }
            [_dataSource addObjectsFromArray:_contactsSource];
            [self hideHud];
        }
        [self.tableView reloadData];
...
- (void)doneAction:(id)sender
{
    ...
    if (_delegate && [_delegate respondsToSelector:@selector(viewController:didFinishSelectedSources:)]) {
        ...
        if ([_blockSelectedUsernames count] == 0) {
            NSLog(@"%@", self.addArray);
            isPop = [_delegate viewController:self didFinishSelectedSources:self.addArray];
        }
        else{
            NSLog(@"addArray - %@", self.addArray);
            NSMutableArray *resultArray = [NSMutableArray array];
            for (NSString *username in self.addArray) {
