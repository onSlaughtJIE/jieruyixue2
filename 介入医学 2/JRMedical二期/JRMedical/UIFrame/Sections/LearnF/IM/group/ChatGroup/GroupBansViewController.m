...
{
    if (_scrollView == nil) {
        _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(10, 10, Width_Screen - 20, kContactSize)];
        _scrollView.tag = 0;
...
//    [self.scrollView removeGestureRecognizer:_longPress];
    ...
    int tmp = (int)([_group.blackList count] + 1) % kColOfRow;
    int row = (int)([_group.blackList count] + 1) / kColOfRow;
    row += tmp == 0 ? 0 : 1;
    self.scrollView.tag = row;
    self.scrollView.frame = CGRectMake(10, 20, Width_Screen - 20, row * kContactSize);
    self.scrollView.contentSize = CGSizeMake(self.scrollView.frame.size.width, row * kContactSize);
    
    if ([_group.blackList count] == 0) {
        return;
    }
    ...
    for (i = 0; i < row; i++) {
        for (j = 0; j < kColOfRow; j++) {
            ...
            if (index < [_group.blackList count]) {
                NSString *username = [_group.blackList objectAtIndex:index];
                ...
                [contactView setDeleteContact:^(NSInteger index) {
                    ...
                    NSArray *occupants = [NSArray arrayWithObject:[weakSelf.group.blackList objectAtIndex:index]];
