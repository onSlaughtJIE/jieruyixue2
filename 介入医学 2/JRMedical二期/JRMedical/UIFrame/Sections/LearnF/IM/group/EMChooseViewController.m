...
{
    if (_tableView == nil) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0, 0, Width_Screen, self.view.frame.size.height) style:UITableViewStylePlain];
...
//    }
    NSArray *arr = [self.dataSource objectAtIndex:section];
    if (!self.showAllIndex && [arr count] == 0)
    {
        return nil;
    }
