...
@interface ReplyPostVC ()<UzysAssetsPickerControllerDelegate,WechatShortVideoDelegate,UICollectionViewDelegate, UICollectionViewDataSource,UITextFieldDelegate,UITextViewDelegate,UIImagePickerControllerDelegate, UINavigationControllerDelegate,UIAlertViewDelegate>
...
- (void)viewDidLoad {
    ...
    self.PLTextView.placeholder = @"详细内容";
    ...
    self.imageCollectView.sd_layout.topSpaceToView(self.PLTextView,8).leftSpaceToView(self.view,8).rightSpaceToView(self.view,10).heightIs(80);
...
- (void)startPublishedPost {
    ...
    if ([self.dataSource count] > 0 ||  ![self.PLTextView.text isEqualToString:@""]) {
        ...
        NSString *params = [NSString stringWithFormat:@"ZICBDYCContent=%@ZICBDYCParentID=%@ZICBDYCPicList=%@",self.PLTextView.text,self.ID,imgVideoJsonStr];
        NSString *url = @"api/Post/ReplyPost";
        
        [self showLoadding:@"正在回帖" time:1000];
        
        [self loadDataApi:url withParams:params block:^(int code, BOOL isSuccess, NSDictionary *modelData) {
            NSLog(@"回帖提交图片/视频 - responseObject - %@---%@", modelData, modelData[@"Msg"]);
            if (isSuccess) {
                ...
                UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"回帖成功"
                                                                    message:message
                                                                   delegate:self
