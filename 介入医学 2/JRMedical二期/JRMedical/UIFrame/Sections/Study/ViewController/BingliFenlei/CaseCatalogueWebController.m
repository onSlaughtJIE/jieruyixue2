//
//  CaseCatalogueWebController.m
//  JRMedical
//
//  Created by ww on 2017/1/17.
...
@interface CaseCatalogueWebController ()<UIWebViewDelegate, UITableViewDelegate, UITableViewDataSource>
{
    NSString *_shareType;
...
@implementation CaseCatalogueWebController
...
- (void)rightBarButtonItemClick {
    ...
    pingLunListVC.isFromCaseCatalogue = YES;
...
- (void)handleToolbar:(UIButton *)sender {
    switch (sender.tag) {
        ...
        case 202:// 点赞
        {
            NSString *params = [NSString stringWithFormat:@"ZICBDYCResourcesType=%@ZICBDYCResourcesID=%@",@"4",self.model.ID];
...
//    [self getRelevantZiXunList]; // 病例分类不要相关资讯
}
...
- (void)getRelevantZiXunList {
    ...
    NSString *params = [NSString stringWithFormat:@"ZICBDYCLableID=%@ZICBDYC",self.LableID];
...
- (void)getCurrentTieziStatus {
    ...
    NSString *url = @"api/CaseCatalogue/GetCaseCatalogueDetails";
    
    [self loadDataApi:url withParams:params block:^(int code, BOOL isSuccess, NSDictionary *modelData) {
        NSLog(@"病例分类- 获取资讯状态 - %@", modelData);
        if (isSuccess) {
...
- (void)getZiXunDetail {
    ...
    NSString *url = @"api/CaseCatalogue/GetCaseCatalogueByID";
    
    [self loadDataApi:url withParams:params block:^(int code, BOOL isSuccess, NSDictionary *modelData) {
        NSLog(@"获病例分类-获取资讯详情 - %@", modelData);
        if (isSuccess) {
