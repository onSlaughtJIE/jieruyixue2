...
@interface MedicalMeetWebDetailVC ()<UIWebViewDelegate, UITableViewDelegate, UITableViewDataSource>
...
@property (nonatomic, assign) NSInteger IsBM;
...
@implementation MedicalMeetWebDetailVC
...
- (void)viewDidLoad {
    ...
    //获取评论列表页面点赞状态的通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(dianZhanZhuangTaiClick) name:@"dianZhanZhuangTai" object:nil];
    
    //获取评论列表页面收藏状态的通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(shouChangZhuangTaiClick) name:@"shouChangZhuangTai" object:nil];
    
    //获取报名成功状态的通知
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(baoMingChengGongClick) name:@"baoMingChengGong" object:nil];
...
- (void)rightBarButtonItemClick {
    NewsPingLunListVC *pingLunListVC = [NewsPingLunListVC new];
    pingLunListVC.model = self.model;
    pingLunListVC.IsDZ = self.IsDZ;
    pingLunListVC.IsShouCang = self.IsShouCang;
    [self.navigationController pushViewController:pingLunListVC animated:YES];
}
...
#pragma mark - 获取评论列表页面点赞状态的通知
-(void)dianZhanZhuangTaiClick {
    [self getCurrentTieziStatus];
}

#pragma mark - 获取评论列表页面收藏状态的通知
-(void)shouChangZhuangTaiClick {
    [self getCurrentTieziStatus];
}

#pragma mark - 获取报名成功状态的通知
-(void)baoMingChengGongClick {
    [self getCurrentTieziStatus];
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.relevantZiXunAry.count > 0) {
        ...
        else if ([groupCode isEqualToString:@"MedicalCongress"]) {//医学会议
            MedicalMeetWebDetailVC *webVC = [[MedicalMeetWebDetailVC alloc] init];
            webVC.model = model;
            [self.navigationController pushViewController:webVC animated:YES];
        }
        else if ([groupCode isEqualToString:@"MedicalCourseware"]) {//医学课件
            // 进入医学课件
...
- (void)addFooterView {
    ...
    NSArray *titleArr = @[@"报名", @"点赞", @"收藏", @"分享"];
    NSArray *imageArr = @[@"baoming", @"hengtianjinfuicon03", @"shoucang_gray", @"fenxiang-1"];
...
- (void)handleToolbar:(UIButton *)sender {
    switch (sender.tag) {
        ...
        {
            if (self.IsBM == 1) {
                return [self showMessage:@"您已经报过名了哦!"];
            }
            MedicineRegistrationVC *mRegistrationVC = [MedicineRegistrationVC new];
            BaseNavigationController *mRegistrationNC = [[BaseNavigationController alloc] initWithRootViewController:mRegistrationVC];
            mRegistrationVC.ID = self.model.ID;
            [self presentViewController:mRegistrationNC animated:YES completion:nil];
        }
            break;
        case 202:// 点赞
        {
...
#pragma mark 分享
        case 204: // 分享
        {
            ...
            if (imageArray) {
                ...
                [shareParams SSDKSetupShareParamsByText:self.model.BiaoTi
                                                 images:@[imageStr]
...
- (void)getCurrentTieziStatus {
    ...
    [self loadDataApi:url withParams:params block:^(int code, BOOL isSuccess, NSDictionary *modelData) {
        ...
        if (isSuccess) {
            self.IsDZ = [modelData[@"JsonData"][@"IsDianZan"] integerValue];
            self.IsShouCang = [modelData[@"JsonData"][@"IsShouCang"] integerValue];
            
            self.IsBM = [modelData[@"JsonData"][@"IsEnroll"] integerValue];
            
            UIButton *bmBtn = (UIButton *)[self.view viewWithTag:201];
            UIButton *dzBtn = (UIButton *)[self.view viewWithTag:202];
            UIButton *scBtn = (UIButton *)[self.view viewWithTag:203];
            
            if (self.IsBM == 0) {
                [bmBtn setTitleColor:RGB(160, 160, 160) forState:(UIControlStateNormal)];
                [bmBtn setImage:[UIImage imageNamed:@"baoming"] forState:(UIControlStateNormal)];
            }
            else {
                [bmBtn setTitleColor:RGB(230, 79, 70) forState:(UIControlStateNormal)];
                [bmBtn setImage:[UIImage imageNamed:@"baomingh"] forState:(UIControlStateNormal)];
            }
...
- (void)setNavBarButtonItem {
    ...
    [imgBtn setTitle:@"评论" forState:UIControlStateNormal];
    [imgBtn setImage:[UIImage imageNamed:@"pinglun"] forState:UIControlStateNormal];
    [imgBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [imgBtn setTintColor:[UIColor whiteColor]];
    [imgBtn addTarget:self action:@selector(rightBarButtonItemClick) forControlEvents:UIControlEventTouchUpInside];
