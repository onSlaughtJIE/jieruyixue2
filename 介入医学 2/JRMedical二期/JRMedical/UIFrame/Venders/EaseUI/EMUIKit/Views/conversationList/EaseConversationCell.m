...
- (void)_setupSubview
{
    ...
    _avatarView.imageView.backgroundColor = [UIColor clearColor]; // jia motify
...
- (void)getUSerMessageWith:(NSString *)str {
    ...
    [AFManegerHelp POST:urlS parameters:paraDic success:^(id responseObjeck) {
        ...
        NSLog(@"EaseConversationCell - GetCustomerInfoByID - %@", responseObjeck);
        if ([responseObjeck[@"Success"] integerValue] == 1) {
            NSArray *array = responseObjeck[@"JsonData"];
            for (NSDictionary *d in array) {

                self.titleLabel.text = d[@"CustomerName"];
                [self.avatarView.imageView sd_setImageWithURL:[NSURL URLWithString:d[@"UserPic"]] placeholderImage:[UIImage imageNamed:@"chatListCellHead"]];
                self.avatarView.imageView.backgroundColor = [UIColor clearColor];
                //
                _model.title = d[@"CustomerName"]; // 检索是根据_model的title查询的
//                _model.avatarURLPath = d[@"UserPic"];
...
//        [self getUSerMessageWith:model.title];
        [self getUSerMessageWith:model.conversation.conversationId];
//        self.titleLabel.text = _model.title;
    ...
    if (model.conversation.type == EMConversationTypeGroupChat) {
        NSLog(@"EaseConversationCell------>>>%@", _model.title);
        self.detailLabel.text = _model.title;
        NSString *string = model.conversation.conversationId;
        EMError *error = nil;
        
        EMGroup *group = [[EMClient sharedClient].groupManager fetchGroupInfo:string includeMembersList:YES error:&error];
        if (group.isPublic) {
            [self.avatarView.imageView sd_setImageWithURL:nil placeholderImage:[UIImage imageNamed:@"guanfangnn"]];
            
        } else {
            [self.avatarView.imageView sd_setImageWithURL:nil placeholderImage:[UIImage imageNamed:@"shehuinn"]];
        }
