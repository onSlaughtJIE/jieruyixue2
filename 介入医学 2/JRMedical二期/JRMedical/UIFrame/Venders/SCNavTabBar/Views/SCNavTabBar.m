...
{
    _items = [@[] mutableCopy];
    _arrowImage = [UIImage imageNamed:SCNavTabbarSourceName(@"arrow_1.png")];
...
//    _line.backgroundColor = UIColorWithRGBA(20.0f, 80.0f, 200.0f, 0.7f);
    _line.backgroundColor = UIColorWithRGBA(0, 164.0f, 156.0f, 0.7f);
...
- (CGFloat)contentWidthAndAddNavTabBarItemsWithButtonsWidth:(NSArray *)widths
{
    ...
    for (NSInteger index = 0; index < [_itemTitles count]; index++)
    {
        ...
        button.tag = index; // add
        if (index == 0) {
            [button setTitleColor:UIColorWithRGBA(0, 164.0f, 156.0f, 0.7f) forState:(UIControlStateNormal)];
        } else {
            [button setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
        } // add
        [button setTitle:_itemTitles[index] forState:UIControlStateNormal];
        button.titleLabel.font = [UIFont systemFontOfSize:15.0]; // add
...
//        NSLog(@"widths - %@", widths);
        ...
        for (int i = 0 ; i <= index; i ++) {
            lineLeft += [widths[i] floatValue];
            if (i != 0) {
                lineLeft += 1;
            }
            
        }
//        UILabel *lineLab = [[UILabel alloc] initWithFrame:(CGRectMake([widths[index] floatValue]*(index+1) + index, 12, 1, 20))];
        UILabel *lineLab = [[UILabel alloc] initWithFrame:(CGRectMake(lineLeft, 12, 1, 20))];
        lineLab.backgroundColor = self.lineColor;
        [_navgationTabBar addSubview:lineLab];
        
        [_items addObject:button];
        buttonX += [widths[index] floatValue]+1;
    }
...
//    _arrowButton.backgroundColor = self.backgroundColor;
    _arrowButton.backgroundColor = [UIColor clearColor];  // motify
